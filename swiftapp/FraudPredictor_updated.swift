...
struct FraudPredictor {
    ...
    func predict(_ batch: ClickBatch) -> Double {
        let input = FraudModelInput(
            ip_clicks_1m: batch.ipClicks1m,
            ip_clicks_5m: batch.ipClicks5m,
            ip_clicks_1h: batch.ipClicks1h,
            device_clicks_1m: batch.deviceClicks1m,
            device_clicks_5m: batch.deviceClicks5m,
            device_clicks_1h: batch.deviceClicks1h,
            ip_fingerprint_entropy_5m: batch.ipFingerprintEntropy5m,
            ip_inter_click_gap_seconds: batch.ipInterClickGapSeconds,
            click_to_install_delta_seconds: batch.clickToInstallDeltaSeconds,
            campaign_conversion_rate: batch.campaignConversionRate,
            publisher_conversion_rate: batch.publisherConversionRate
        )
